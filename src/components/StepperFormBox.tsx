...
export const StepperFormBox: FC<StepperFormBoxProps> = ({
  children,
  title,
  description,
  titleRef,
        ...
        <CardContent className="p-8 grid gap-10">
          {(title || description) && (
            <div className="space-y-2">
              {title && (
                <h2 ref={titleRef} className="text-lg font-semibold font-pixel">
                  {title}
                </h2>
              )}
              {description && (
                <p ref={descriptionRef} className="text-sm text-gray-600">
                  {description}
                </p>
              )}
            </div>
